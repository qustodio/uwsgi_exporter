...
func init() {
	prometheus.MustRegister(versioncollector.NewCollector("uwsgi_exporter"))
}
...
func main() {
	...
	if err != nil {
		logger.Error("Failed to listen address", "error", err)
		os.Exit(1)
	}
}
...
func newHandler(metrics collector.Metrics, logger *slog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if *statsURI != "" {
			...
			if err != nil {
				logger.Error("Failed to create stats reader", "error", err)
